...
searchBtn.addEventListener("click",()=>{
    ...
    fetch(finalURL).then((response)=>response.json()).then((data)=>{
        ...
        <div class="wrapper">    
            <div class="data-wrapper">
                <h4>Currency:</h4>
                <span>${currency.name} => ${Object.keys(data[0].currencies)[0]}, Symbol: ${currency.symbol} </span>
            </div>  
        </div>   
        <div class="wrapper">   
            <div class="data-wrapper">
    ...
    .catch(()=>{
        if(countryName.length==0){
            finalresult.innerHTML=`<h3>The input field cannot be empty</h3>`;
        }
        else{
            finalresult.innerHTML=`<h3>Please enter a valid country name</h3>`;
        }
    });
});